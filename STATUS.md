...
2. **Translation** — **§1 + §2 SHIPPED.** §1 end-of-meeting transcript translation (`83687ad`): saved-card "Translate" → pick a language → `llm.translate` (cloud redacts / local zero-egress) → Save-to-note (`## Transcript — <lang>`); privacy-audited. §2 live → English: a `→ EN` controls-bar toggle flips WhisperKit to `task: .translate` (any source → English captions), pure on-device, zero egress, no model needed. **Remaining:** §3 live → arbitrary target (MT model / Apple Translation / per-segment LLM — its own ADR). See BACKLOG.
