...
**Current phase:** Phase 5 diarization v1 **shipped** · **offline** FluidAudio (`OfflineDiarizerManager`, VBx) speaker labels + engine-written diarized transcripts committed to the vault · utterance **supersession** + at-stop dedup clean both live captions and the saved file · Phase 4 UI surfaces largely in place (live transcript, tray, Settings, prefs) · next: validate on a real multi-speaker meeting recording, then transcript-readability fix + Phase 5.1 speaker enrollment/naming
...
- **Phase 4 follow-up — live harkd path validated with Process Taps + lazy permissions** ([ADR-0012](docs/decisions/0012-harkd-lazy-permissions-startup.md)). The full product loop runs end-to-end: Bluetooth system audio → Process Tap → energy VAD → sliding window → WhisperKit → WebSocket `segment.partial`/`final`, verified on M1/macOS 26.5 (2026-05-31) with accurate English transcription, RTF 0.38, and **stable `utterance_id` across partial→final in the live path** (ADR-0009 holds, not just in unit tests). Fixed three daemon UX warts found in testing: (1) harkd no longer **fail-fast-exits** at startup — it required Screen Recording, which the tap backend doesn't need and which can't grant without a process restart, so granting left no process to continue; permissions are now requested **lazily at `capture.start`** (mic via `PermissionGate.requestMicrophone`; system audio via the tap SPI), both live-granting → grant-and-continue, no relaunch. (2) The WS server + `engine.port` file now come up **before** the slow model load (model loads behind a readiness gate; `capture.start` before ready → recoverable `ENGINE_WARMING_UP`), so the daemon is discoverable instantly. (3) Doc: `engine.port` is JSON `{pid,port,version}` by design (ADR-0008) — manual `websocat` must parse the `port` field, not `cat` it raw. Dev tooling: `sign-dev-bundle.sh` now wraps `harkd` too (`com.hark.daemon.dev`).
...
- **Phase 5 — diarization v1 SHIPPED** ([ADR-0016](docs/decisions/0016-phase-5-diarization.md), pipeline now **offline** per [ADR-0017](docs/decisions/0017-diarization-offline-pipeline.md)). Offline, on-device speaker diarization with anonymous "Speaker N" labels — **on-device verified and committed**. The engine now advertises `capabilities: ["diarization"]` in `meta.hello` (commit `5edd223`).
  - **FluidAudio diarization** (slices `0ee846f` + `e48a79d`, pipeline switched to offline in `8e2863a`). FluidAudio pinned at **0.14.8**; the at-stop pass runs FluidAudio's **`OfflineDiarizerManager`** (VBx global clustering over overlapping windows + overlap-aware segmentation + exclusive-segment reconstruction), **not** the streaming `DiarizerManager`. Reason: ~2.5× lower DER on AMI (the streaming path was mislabeling rapid speaker turns, e.g. 69 segments for 239 utterances) — [ADR-0017](docs/decisions/0017-diarization-offline-pipeline.md) supersedes ADR-0016's pipeline choice only; the rest of ADR-0016 stands. Anonymous "Speaker N" labels assigned to final utterances by **time-overlap at `capture.stop`**; models cache under `~/Library/Application Support/Hark/Models/speaker-diarization-coreml/` (rule #2 holds — models cache, not user content). Tuning knobs exist: `HARK_DIAR_THRESHOLD` (clustering threshold) / `HARK_DIAR_NUM_SPEAKERS` (force speaker count, default auto). Verified on-device (speakers correct, **RTF ~0.02**).
  - **Engine writes the diarized transcript to the vault** (commit `e48a79d`). The engine (not Electron — the ADR-0015 → engine-writer migration is now executed, see below) writes markdown to `~/Documents/vault/hark/meetings/YYYY-MM-DD-HHMM.md` (front-matter + `> **Speaker N** · HH:MM:SS` blockquotes), **atomic + never-overwrite**. Local **git init + commit per meeting** (`Hark <hark@localhost>` identity, `.gitignore` excludes `.speakers/`, hooks isolated via `core.hooksPath=/dev/null`, **no remote / no push**). Emits the `meeting.saved` frame; the UI shows a **"Meeting saved — N speakers"** toast. **Privacy-audited clean.** Verified on-device (file written + committed).
- **Phase 5 follow-up — utterance SUPERSESSION + at-stop dedup** (commit `8e2863a`, [ADR-0018](docs/decisions/0018-utterance-supersession-signal.md), extends [ADR-0009](docs/decisions/0009-utterance-id-overlap-rule-v2.md)). New `segment.superseded` Engine→UI wire frame (`utterance_id` + `superseded_by`): when a grown re-segmentation supersedes an earlier overlapping fragment, the engine retracts the old `utterance_id`, the UI deletes it, and the at-stop vault write filters it out — cleaning **both** live captions and the saved file. The supersession gate is **time-containment AND text-prefix/superset — never text alone**, so legitimately repeated phrases (same words at a different, non-overlapping time) are preserved. Plus an at-stop retention dedup (by `utterance_id`, exact-text within ~1s, sorted by `t_start`). **22 engine tests passing**, including a repeat-preservation regression guard (`testLegitimateRepeatAtDifferentTimeNotSuperseded`).

### ⏳ Next up: remaining Phase 5 + Phase 4 surfaces (incremental)

**Recommended next validation:** run a **real multi-speaker meeting recording** (pauses + turn-taking) end-to-end. The shipped offline diarization + supersession were validated on-device, but mostly against adversarial continuous narration; real meeting audio segments far more cleanly and is the representative case to confirm before further tuning.

**Remaining Phase 5 work** (diarization v1 shipped; these turn "Speaker N" into real names + close the open questions):

- **Transcript READABILITY on continuous, pause-less, repetitive narration** — the saved transcript still has residual duplication / run-on fragments that the post-hoc dedup + supersession (ADR-0018) don't fully catch. The robust fix is a **source-level finalization-strategy change**: finalize each sentence once and dedup against finalized history at the source, rather than cleaning up after the fact. Largely an adversarial-input edge — real meeting audio (pauses + turn-taking) segments far more cleanly — but tracked as a follow-up.
- **Clustering-threshold tuning** — speaker boundaries can mis-attribute on quick back-and-forth. Tune via the `HARK_DIAR_THRESHOLD` / `HARK_DIAR_NUM_SPEAKERS` env knobs on a known multi-speaker recording ([ADR-0017](docs/decisions/0017-diarization-offline-pipeline.md) flags the `OfflineDiarizerManager` defaults as needing confirmation on real meeting audio).
...
- ✅ ~~Bookmark button~~ — wired top-bar Bookmark button end-to-end against `bookmark.create` → `bookmark.created` (commit `1e4d6a6`).
- ✅ ~~Warning banner~~ — `StatusBanner` atom landed and wired to engine `warning` frames (e.g. `rtf_high`); renderer no longer swallows them (commit `a6eb818`).
...
Estimated effort: Phase 5 diarization v1 (now **offline**), transcript→vault persistence, and utterance supersession + dedup are all shipped. The remaining Phase 5 work is the transcript-readability finalization fix + clustering-threshold tuning + Phase 5.1 enrollment/naming (which also unblocks the speaker-tagging UI), with a real multi-speaker meeting recording as the next validation. Then Phase 6 (Claude summaries / Q&A) and Phase 7 (packaging). Q&A and the 3-column layout's Q&A column remain gated on Phase 6; the leftover atoms and packaging are still open.
...
17. ~~**Tap privacy flag dropped**~~ — **resolved 2026-05-31.** On-device test (`HARK_TAP_PRIVATE=1`) confirmed `isPrivate=true` alone runs fine — `isExclusive` was the sole blocker — so the tap is now created **private by default** ([ADR-0011](docs/decisions/0011-process-tap-system-audio-gotchas.md)). Tap isolation regained; the experiment env var was removed.
18. **Default capture backend undecided** ([ADR-0011](docs/decisions/0011-process-tap-system-audio-gotchas.md) open Q). Process Taps now work AND the `harkd` live path is validated with them (ADR-0012). ScreenCaptureKit is still the default; Process Taps are opt-in via `HARK_CAPTURE_BACKEND=tap`. Decide whether to flip the default (and demote SCK to fallback-only) once the tap path has more on-device mileage. **Note:** SCK can never grant-and-continue (Screen Recording needs a restart), so flipping to taps also improves first-run UX — a point in favor.
19. **harkd readiness push frame** ([ADR-0012](docs/decisions/0012-harkd-lazy-permissions-startup.md) open Q). `capture.start` before the model loads returns recoverable `ENGINE_WARMING_UP`; the client must retry. A `meta.ready` / updated `meta.hello` broadcast on `attachModel` would let the UI start without polling. Deferred until the Phase 4 UI needs it.
...
- ✅ Diarization **pipeline = offline `OfflineDiarizerManager` (VBx)**, not the streaming `DiarizerManager` — locked by [ADR-0017](docs/decisions/0017-diarization-offline-pipeline.md) (supersedes ADR-0016's pipeline choice only; ~2.5× lower DER on AMI, same library/models). Don't revisit the streaming path.
- ✅ Utterance supersession — locked by [ADR-0018](docs/decisions/0018-utterance-supersession-signal.md) (extends ADR-0009): the `segment.superseded` retraction signal, gated on **time-containment AND text-prefix — never text-similarity alone**. Don't loosen the gate to text-only (it would eat legitimate repeats); that needs a new ADR + hostile-input review.
