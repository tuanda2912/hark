...
1. **On-device testing (user) — full checklist: [docs/ON-DEVICE-TEST-CHECKLIST.md](docs/ON-DEVICE-TEST-CHECKLIST.md).** Exercise summary / this-meeting Q&A / vault Q&A / translation §1·§2·§3 with a real API key (Anthropic / OpenAI-compatible) **or** a local Ollama (`baseUrl` → zero egress). §3 live → arbitrary target is the newest and is best tested on the local model (proves zero egress). Built-in vault RAG works on a clean install (embedder hosted — see above); external RAG: a ~10-line local stub server proves the path (see BACKLOG / the 4c–external report).
2. **Translation** — **§1 + §2 SHIPPED; §3 IMPLEMENTED (on-device confirmation pending).** §1 end-of-meeting transcript translation (`83687ad`): saved-card "Translate" → pick a language → `llm.translate` (cloud redacts / local zero-egress) → Save-to-note (`## Transcript — <lang>`); privacy-audited. §2 live → English: a `→ EN` controls-bar toggle flips WhisperKit to `task: .translate` (any source → English captions), pure on-device, zero egress, no model needed. §3 live → arbitrary target ([ADR-0035](docs/decisions/0035-live-translation-arbitrary-target.md), **Option C**): a non-English target picker next to `→ EN` (mutually exclusive, locked during capture) translates each FINALIZED line via the configured LLM in main — `LiveTranslationService` → `llm.translateSegment` → `segment.translation` (bilingual live view). Reuses §1's egress governance (cloud redacts each line / **local = zero egress**); per-segment cloud egress aggregated into ONE metadata-only `translate-live` cloud-log entry; engine unchanged, no model shipped. **Automated gates green** (build + privacy-audit PASS); **needs the user:** live non-English audio → translated captions + latency feel + a local-model zero-egress run.
