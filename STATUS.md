...
**Current phase:** **Phase 6 (LLM + vault RAG) shipped; translation finalized.** Provider-agnostic LLM (meeting summary · this-meeting Q&A · vault-wide Q&A) + **pluggable retrieval** (built-in on-device RAG **and** external loopback MCP/HTTP backend), all behind the local-first egress governance. **Translation is now on-demand, post-stop only** — live (translate-during-capture) translation was removed/deferred ([ADR-0037](docs/decisions/0037-defer-live-translation.md)); the saved transcript gets a faithful per-utterance structured mirror with export-only grow-in-place completeness ([ADR-0036](docs/decisions/0036-grow-in-place-finalization.md)). Phase 5 / 5.1 (offline diarization + speaker enrollment + privacy model) shipped. **Next:** **Phase 7 — packaging / notarization** (app icon · TCC attribution · notarize once a Developer ID is available) · remaining on-device testing.
...
### ✅ Done — Translation finalized + transcript completeness (2026-06-04)

- **Live translation removed / deferred ([ADR-0037](docs/decisions/0037-defer-live-translation.md)).** Both the on-device `→ EN` toggle (§2) and the per-line LLM arbitrary-target picker (§3) are gone — `LiveTranslationService` deleted, auto-translate-on-stop dropped, the live hooks (`segmentFinalized$`, `setSegmentTranslation`) removed. It was hard to test, timeout-prone on a small local model, and rendered messily. The engine's `task:.translate` plumbing is left **dormant** for an easy revival. **Translation is now on-demand, post-stop only.**
- **On-demand structured translate.** The saved-card **Translate** panel was rewired from the legacy whole-transcript blob to the structured background job (`TranslationJobService` → `EngineService.writeTranslationLines`): per-utterance translation with a progress banner, persisted as a `## Transcript — <lang>` section that is a byte-for-byte structural mirror of the original (same labels / wall-clock / blockquote). Local = zero egress; cloud redacts each line + ONE aggregated metadata-only log entry. Discloses egress before sending. Privacy-audited PASS.
- **Export-only grow-in-place finalization ([ADR-0036](docs/decisions/0036-grow-in-place-finalization.md)).** A fuller re-decode of an already-finalized utterance now extends the RETAINED row for the SAVED transcript **without re-broadcasting** — recovers dropped tails (content-loss fix) while the live view stays discrete. Conservative time+text-prefix gate; amends ADR-0019. **Engine: 191 tests, 0 failures.**
- **"Reveal in Finder" lands on the meeting note** (new vault-descendant-gated, reveal-only `hark:reveal-path` IPC) instead of the vault root; the saved-card footer **wraps** instead of ballooning past its width.
- Shipped as `6d8a353` (engine, ADR-0036) + `aa5a2ea` (UI, ADR-0037), pushed to `main`. On-device tested by the user.
...
1. **Phase 7 — packaging / notarization (CURRENT).** electron-builder + sign the Swift sidecar + hardened-runtime entitlements + `kTCCServiceAudioCapture` attribution. **~60% done** (`ui/electron-builder.yml`, `build/notarize.js` no-op-without-creds, entitlements, sidecar bundling, a signed dev `Hark.app`). **Remaining:** an **app icon** (`ui/build/icon.icns`); a **TCC-attribution check** on a signed build (the mic / system-audio prompt must say **"Hark"**, not "Electron" — the point of the sidecar `cs.inherit` entitlement); and the **notarize / staple** last mile — which needs a **paid Apple Developer Program membership** ($99/yr) + a **Developer ID Application** cert (the current free "Apple Development" cert signs but **can't** notarize). Onboarding polish. See [docs/BACKLOG.md](docs/BACKLOG.md) "Packaging & distribution".
2. **On-device testing (user) — remaining checklist: [docs/ON-DEVICE-TEST-CHECKLIST.md](docs/ON-DEVICE-TEST-CHECKLIST.md).** Beyond the translation rework (tested): meeting summary · this-meeting Q&A · vault-wide RAG Q&A, with a real API key (Anthropic / OpenAI-compatible) **or** a local Ollama (`baseUrl` → zero egress). Built-in vault RAG works on a clean install (embedder hosted); external RAG: a ~10-line local stub proves the path.
3. **Polish / deferred** (all in BACKLOG): jump-to-source citations (chunks carry `note_path` + offsets); live index-status for the external backend; richer MCP (resources/streaming/SDK); a committed renderer/main **test runner** (only Swift + throwaway smokes today); clustering-threshold tuning; redaction NER + toggle. **Reviving live translation** ([ADR-0037](docs/decisions/0037-defer-live-translation.md)) — needs an on-device MT model + decoupling from the finalization watermark.
...
5. **Transcript readability** on continuous, pause-less narration — residual run-on/dup the post-hoc dedup doesn't fully catch; robust fix is source-level finalize-once. **Partially addressed by [ADR-0036](docs/decisions/0036-grow-in-place-finalization.md)** (export-only grow-in-place recovers dropped tails in the SAVED transcript); residual live churn remains. Mostly an adversarial-input edge (real meetings segment cleanly).
